function GetLeft(elm) {
  ...
  while ((elm = elm.offsetParent)) left += elm.offsetLeft;
...
function GetTop(elm) {
  ...
  while ((elm = elm.offsetParent)) top += elm.offsetTop;
...
Array.prototype.InsertAt = function(arg, position) {
  ...
  for (var i = 0; i < arr1.length; i++) this.push(arr1[i]);

  this.push(arg);

  for (var j = 0; j < arr2.length; j++) this.push(arr2[j]);
};

Array.prototype.Contains = function(arg) {
  for (var i = 0; i < this.length; i++) if (this[i] == arg) return true;
...
Vector2 = function(x, y) {
  ...
  this.Difference = function(vec2, invert) {
    ...
    else return new Vector2((this.x - vec2.x) * inv, (this.y - vec2.y) * inv);
  };
};
...
Rectangle = function(x, y, w, h, color) {
  if (x == null || y == null || w == null || h == null) {
    alert(
      "You must pass in all the veriables for a rectange: (x, y, width, height)"
    );
...
Animation = function(
  ...
  if (limit == null || limit == 0) this.limit = 999999999999;
  ...
  this.Draw = function(ctx) {
    ...
    if (this.fpsCounter >= this.fps) this.fpsCounter = 0;
  };
};
